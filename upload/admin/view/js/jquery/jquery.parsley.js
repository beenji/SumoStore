  ...
  ParsleyForm.prototype = {
    ...
    onSubmitValidate: function (event) {
      ...
      if (false === this.validationResult && event instanceof $.Event) {
        ...
        // uber hack.... [tag: edit by chris]
        console.log('error on, searching for error');
        var errorDiv = $('.' + ParsleyDefaults.errorClass);
        if (errorDiv.closest('.tab-pane').is(':hidden')) {
          $('a[href="#' + errorDiv.closest('.tab-pane').attr('id') + '"]').trigger('click');
        }
        else {
          if (errorDiv.parent().hasClass('lang-block') && !errorDiv.parent().hasClass('lang-active')) {
            // comes from core.js
            var language = errorDiv.parent().attr('class');
            language = language.replace('input-group', '');
            language = language.replace('lang-block', '');
            language = language.replace('lang-active', '');
            language = language.replace('lang-', '');
            // make sure no whitespace is there
            language = language.replace(' ', '');
            language = language.replace(' ', '');
            language = language.replace(' ', '');
            // trigger language selector
            $('#language-selector a[data-lang-id="' + language + '"]').trigger('click');
          }
        }
      }
      return this;
