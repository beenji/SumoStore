...
class ModelLocalisationLengthClass extends Model
{
	...
	public function getLengthClasses($data = array())
	{
		if ($data) {
			...
			$length_class_data = Cache::find('length_class.' . (int)$this->config->get('language_id'));

			if (!$length_class_data) {
				$length_class_data = $this->fetchAll(
				    "SELECT *
				    FROM PREFIX_length_class lc
				    LEFT JOIN PREFIX_length_class_description lcd
				    	ON (lc.length_class_id = lcd.length_class_id)
				    WHERE lcd.language_id = '" . (int)$this->config->get('language_id') . "'");

				Cache::set('length_class.' . (int)$this->config->get('language_id'), $length_class_data);
			}
