...
final class Tax
{
    ...
    public function getRateName($tax_rate_id) {
        $tax_query = Database::query("SELECT name FROM PREFIX_tax_rate WHERE tax_rate_id = '" . (int)$tax_rate_id . "'")->fetch();
    ...
    public function getRates($value, $tax_class_id)
    {
        ...
        $cacheFile = 'products.tax_rate.' . $customer_group_id . '.' . $tax_class_id . '.';

        if ($this->shipping_address) {
            $file = $cacheFile . $this->shipping_address['country_id'] . '.' . $this->shipping_address['zone_id'];

            $results = null; //Cache::find($file);
            if (!is_array($results)) {
                $results = Database::fetchAll("SELECT tr2.tax_rate_id, tr2.name, tr2.rate, tr2.type, tr1.priority FROM PREFIX_tax_rule tr1 LEFT JOIN PREFIX_tax_rate tr2 ON (tr1.tax_rate_id = tr2.tax_rate_id) INNER JOIN PREFIX_tax_rate_to_customer_group tr2cg ON (tr2.tax_rate_id = tr2cg.tax_rate_id) LEFT JOIN PREFIX_zone_to_geo_zone z2gz ON (tr2.geo_zone_id = z2gz.geo_zone_id) LEFT JOIN PREFIX_geo_zone gz ON (tr2.geo_zone_id = gz.geo_zone_id) WHERE tr1.tax_class_id = '" . (int)$tax_class_id . "' AND tr1.based = 'shipping' AND tr2cg.customer_group_id = '" . (int)$customer_group_id . "' AND z2gz.country_id = '" . (int)$this->shipping_address['country_id'] . "' AND (z2gz.zone_id = '0' OR z2gz.zone_id = '" . (int)$this->shipping_address['zone_id'] . "') ORDER BY tr1.priority ASC");
        ...
        if ($this->payment_address) {
            $file = $cacheFile . $this->payment_address['country_id'] . '.' . $this->payment_address['zone_id'];
            $results = null; //Cache::find($file);
            if (!is_array($results)) {
        ...
        if ($this->store_address) {
            $file = $cacheFile . $this->store_address['country_id'] . '.' . $this->store_address['zone_id'];
            $results = null; //Cache::find($file);
            if (!is_array($results)) {
