...
use Sumo;
use App;
class ControllerShippingweight extends App\Controller
{
    public function index()
    {
        ...
        $this->document->addBreadcrumbs(array(
            'text' => Sumo\Language::getVar('APP_SHIPPINGWEIGHT_TITLE')
        ));
    ...
    {
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['store_id']) && isset($this->request->post['copy_id'])) {
            $this->load->appModel('Settings');
            $data = $this->shippingweight_model_settings->getSettings($this->request->post['copy_id']);
            $data['enabled'] = 'on';
            $this->shippingweight_model_settings->setSettings($this->request->post['store_id'], $data);
