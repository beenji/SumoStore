...
use Sumo;
use App;
class ControllerShippingflat extends App\Controller
{
    public function index()
    {
        $this->document->setTitle(Sumo\Language::getVar('APP_SHIPPINGFLAT_TITLE'));
        $this->document->addBreadcrumbs(array(
            'href' => $this->url->link('common/apps', '', 'SSL'),
            'text' => Sumo\Language::getVar('SUMO_ADMIN_APPS_DASHBOARD')
        ));
        $this->document->addBreadcrumbs(array(
            'text' => Sumo\Language::getVar('APP_SHIPPINGFLAT_TITLE')
        ));
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            $this->shippingflat_model_settings->setSettings($this->data['current_store'], $this->request->post['settings']);
            //$this->redirect($this->url->link('app/shippingflat', '', 'SSL'));
        }
        $this->data['settings'] = $this->shippingflat_model_settings->getSettings($this->data['current_store']);
