...
use Sumo;
use App;
class ControllerShippingfree extends App\Controller
{
    public function index()
    {
        $this->document->setTitle(Sumo\Language::getVar('APP_SHIPPINGFREE_TITLE'));
        $this->document->addBreadcrumbs(array(
            'href' => $this->url->link('common/apps', '', 'SSL'),
            'text' => Sumo\Language::getVar('SUMO_ADMIN_APPS_DASHBOARD')
        ));
        $this->document->addBreadcrumbs(array(
            'text' => Sumo\Language::getVar('APP_SHIPPINGFREE_TITLE')
        ));
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            $this->shippingfree_model_settings->setSettings($this->data['current_store'], $this->request->post['settings']);
            $this->redirect($this->url->link('app/shippingfree', '', 'SSL'));
        }
        $this->data['settings'] = $this->shippingfree_model_settings->getSettings($this->data['current_store']);
