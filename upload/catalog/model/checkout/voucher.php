...
class ModelCheckoutVoucher extends Model
{
    public function check($code)
    {
        $data = $this->query(
            ...
            FROM PREFIX_voucher AS v
            LEFT JOIN PREFIX_voucher_theme_description AS vtd
                ON vtd.voucher_theme_id = v.voucher_theme_id
            WHERE code = :code
                AND status = 1
                AND vtd.language_id = :lang",
            array(
                'code'  => $code,
        ...
        if (is_array($data) && count($data)) {
            $history = $this->fetchAll("SELECT amount FROM PREFIX_voucher_history WHERE voucher_id = :id", array('id' => $data['voucher_id']));
