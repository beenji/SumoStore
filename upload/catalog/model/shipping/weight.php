...
{    
    public function getQuote($address) 
    {
            ...
            if ($status) {
                ...
                if ((string)$cost != '') { 
                    $quote_data['weight_' . $result['geo_zone_id']] = array(
                        ...
                        'title'        => $result['name'] . '  (' . Language::getVar('SUMO_NOUN_WEIGHT') . ' ' . $this->weight->format($weight, $this->config->get('config_weight_class_id')) . ')',
                        'cost'         => $cost,
        ...
        if ($quote_data) {
            $method_data = array(
                'code'       => 'weight',
                'title'      => Language::getVar('SUMO_SHIPPING_WEIGHT_TITLE'),
                'quote'      => $quote_data,
